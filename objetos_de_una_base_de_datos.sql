...
-- realizar una vista con los datos que contienen nulos en la foreign key
CREATE VIEW view_marketing_troop_null AS 
(SELECT 
   f.ID
   ,concat(f.FIRST_NAME, " ",f.LAST_NAME) AS NOMBRE_COMPLETO
   ,f.TROOP 
FROM CLASE7.FRIENDS AS f
LEFT JOIN CLASE7.TROOP AS t
   ON f.TROOP = t.ID
WHERE f.TROOP IS NULL);

-- traer los valores de la vista
SELECT *
FROM view_marketing_troop_null;
...
CALL InsertFriendWithTroop("pirulo","pirulin",1);

SELECT * FROM clase7.friends;
